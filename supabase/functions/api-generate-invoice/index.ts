...
// Example:
//   curl -X POST .../api-generate-invoice \
//     -H "x-api-key: biz_live_xxx" -H "Content-Type: application/json" \
//     -d '{"prompt":"Invoice for 10 hrs dev at $80/hr for Acme, tax 8%"}'
// ════════════════════════════════════════════════════════════════
...
// ---- Provider calls (shared with ai-automation) ----
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: profile } = await service.from("profiles").select("ai_provider, api_usage_count, api_usage_limit, plan, trial_ends_at").eq("id", keyRow.user_id).single();
    ...
    const result = await callAI(
      ...
      "You are an expert billing assistant. Generate a complete invoice as valid JSON with keys: invoice_number, client_name, client_email, client_address, items (array of {description, quantity, unit_price}), tax_rate, due_date, notes. Calculate subtotal, tax_amount, total automatically. Return ONLY valid JSON.",
      prompt
    );

    await service.rpc("increment_api_usage", { user_uuid: keyRow.user_id });
    await service.from("activity_logs").insert({ user_id: keyRow.user_id, action_type: "invoice", description: "API: generate-invoice", time_saved_minutes: 15, money_saved: 7.50, provider: profile?.ai_provider });

    return new Response(JSON.stringify({ invoice_raw: result }), { headers: cors });
  } catch (e) {
